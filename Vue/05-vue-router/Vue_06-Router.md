...
- childen 옵션은 배열 형태로 필요한 만큼 중첩 관계를 표현할 수 있음!
...
- 다른 위치로 이동하기 (Navigation to a different location)
...
- "Login"이 되어있지 않다면 페이지 진입을 막고 Login 페이지로 이동시키기
...
> 전역 가드 beforeEach 관련 코드는 주석 처리 후 진행 -> 전역처리가 먼저 걸려서 화면간 이동이 불가능 하기 때문
